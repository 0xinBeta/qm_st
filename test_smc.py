...
def main():
    ...
    # Calculate EMA (since it was referenced in your code)
    df['EMA'] = df['Close'].ewm(span=15, adjust=False).mean()
    

    # Calculate signals based on EMA
    calculate_signals(df)

    # Identify pivots
    window = 5
    df['isPivot'] = df.apply(lambda x: isPivot(df, x.name, window), axis=1)

    # Detect structure
    df['pattern_detected'] = df.index.map(lambda x: detect_structure(df, x, backcandles=40, window=6))

    df.to_csv("test_smc.csv")
